...
var username : set = username_set
var username_text_instance = null

puppet var puppet_hp = 100: set = puppet_hp_set
puppet var puppet_position = Vector2(0, 0): set = puppet_position_set
puppet var puppet_velocity = Vector2()
puppet var puppet_rotation = 0
puppet var puppet_username = "": set = puppet_username_set
...
func _ready():
	get_tree().connect("peer_connected", Callable(self, "_network_peer_connected"))
	...
	if get_tree().has_multiplayer_peer():
		if is_multiplayer_authority() and visible:
			...
			if Input.is_action_pressed("click") and can_shoot and not is_reloading:
				rpc("instance_bullet", get_tree().get_unique_id())
				is_reloading = true
				reload_timer.start()
		else:
...
func username_set(new_value) -> void:
	...
	if get_tree().has_multiplayer_peer():
		if is_multiplayer_authority() and username_text_instance != null:
...
func puppet_username_set(new_value) -> void:
	...
	if get_tree().has_multiplayer_peer():
		if not is_multiplayer_authority() and username_text_instance != null:
...
@rpc("any_peer", "call_local") func instance_bullet(id):
	...
	player_bullet_instance.set_multiplayer_authority(id)
	player_bullet_instance.player_rotation = rotation
	player_bullet_instance.player_owner = id
	Network.networked_object_name_index += 1

@rpc("any_peer", "call_local") func update_position(pos):
	global_position = pos
	puppet_position = pos
...
@rpc("any_peer", "call_local") func enable() -> void:
	hp = 100
	can_shoot = false
	update_shoot_mode(false)
